...
function PostFormPage({ mode }: { mode: Mode }) {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      if (p) {
        if (author && p.author_id !== author.id) {
  ...
  }, [mode, editingId, author, navigate]);
  ...
  async function handleSubmit(e: FormEvent) {
    ...
    if (!author?.id) return toast.error("Author profile not loaded");
    setSaving(true);
    ...
    if (mode === "new") {
      ...
      navigate({ to: "/author/posts/$id/edit", params: { id: data.id } });
    } else if (editingId) {
  ...
  return (
    <>
      <div className="flex items-center justify-between mb-6">
        <h1 className="font-display text-3xl font-bold">{mode === "new" ? "New post" : "Edit post"}</h1>
        <button
          type="button"
...
export const Route = createFileRoute("/author/posts/new")({
  head: () => ({ meta: [{ title: "New post - Author" }, { name: "robots", content: "noindex" }] }),
  component: () => (
    <AuthorGuard>
      <AuthorLayout>
